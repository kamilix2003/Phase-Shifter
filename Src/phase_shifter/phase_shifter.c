...
phase_shifter_StatusTypeDef phase_shifter_init(phase_shifter_TypeDef *hps, SPI_HandleTypeDef *hspi, DMA_HandleTypeDef *hdma, GPIO_TypeDef *latch_port, uint16_t latch_pin){
    if(hps == NULL || hspi == NULL || hdma == NULL || latch_port == NULL) return PHASE_SHIFTER_ERROR;
...
phase_shifter_StatusTypeDef phase_shifter_set(phase_shifter_TypeDef *hps, uint8_t *data, size_t size){
    ...
    if(hps->hspi == NULL || hps->hdma == NULL) return PHASE_SHIFTER_ERROR;

    while(hps->hspi->State == HAL_SPI_STATE_BUSY || hps->hdma->State == HAL_DMA_STATE_BUSY);
    ...
    if(HAL_SPI_Transmit_DMA(hps->hspi, hps->buffer, PHASE_SHIFTER_COUNT) != HAL_OK){
